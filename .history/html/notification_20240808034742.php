...
$stmt = $pdo->prepare("SELECT * FROM messages WHERE userID = ?");
$stmt->execute([$_SESSION['receiverID']]);
...
<body>
    ...
    <?php
        foreach ($messages as $message) {
            //sender name
            $sender=$pdo->prepare("SELECT name FROM users WHERE senderID = ?");
            $sender->execute([$message['senderID']]);
            $senderName = $sender->fetch(PDO::FETCH_ASSOC);
            $Sname = $senderName['name'];
            

            echo "<h1>New Invitation</h1>";
            echo "<p class='message'>You have a new invite from <strong>{$Sname}</strong>.</p>";
            echo "<a href='{$message['link']}'>Enter</a>";
        }
    ?>
