            ...
            <div id="results">
                <!-- <?php foreach ($results as $buddy): ?>
                    <div class="result">
                        ...
                        <a href="chat.php?buddy=<?= urlencode($buddy['name']); ?>&buddyid=<?= htmlspecialchars($buddy['uid']); ?>">Chat with <strong>Buddy</strong></a>
                   </div>
                <?php endforeach; ?> -->
                <?php 
                foreach ($results as $buddy) {
                    echo "<div class='result'>";
                    echo "<strong>Name:</strong> " . htmlspecialchars($buddy['name']) . "<br>";
                    echo "<strong>Major:</strong> " . htmlspecialchars($buddy['major'] ?? 'N/A') . "<br>";
                    echo "<strong>Hours to Study:</strong> " . htmlspecialchars($buddy['hoursToStudy'] ?? 'N/A') . "<br>";
                    echo "<strong>Stress Level:</strong> " . htmlspecialchars($buddy['stressLevel'] ?? 'N/A') . "<br>";
                    echo "<strong>Interest:</strong> " . htmlspecialchars($buddy['interest'] ?? 'N/A') . "<br>";
                    echo "<strong>Date:</strong> " . htmlspecialchars($buddy['date'] ?? 'N/A') . "<br>";
                    echo "<a href='rating.php?buddy=" . urlencode($buddy['name']) . "'>Rate this Buddy</a>";
                    echo "<a href='chat.php?buddy=" . urlencode($buddy['name']) . "&buddyid=" . htmlspecialchars($buddy['userID']) . "'>Chat with <strong>Buddy</strong></a>";
                    echo "<a href='conference.php?buddy=" . urlencode($buddy['name']) . "&buddyid=" . htmlspecialchars($buddy['userID']) . "'>Call <strong> Buddy</strong></a>";
                    echo "</div>";
                }
                ?>
            </div>
